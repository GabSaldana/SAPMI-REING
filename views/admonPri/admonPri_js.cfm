...
function eliminaUsuario(){
    ...
    $.post('/index.cfm/admonPri/admonPri/eliminaUsuario', {
            pkUsu: pk,
        }, function(data){
            if(data > 0){
                consultarUsuarios();
...
function actualizarUsuario(edo, successMsg, errorMsg) {
    var pk = $('#mdl-estado-usuario').find('p').attr('pkUsr');
    $("#mdl-estado-usuario").modal('hide');
    $.post('admonPri/actualizarUsuario', {
            pkUsu: pk, estado: edo
        }, function(data) {
        if(data > 0) {
            consultarLista();
...
$(document).ready(function() {
    ...
    * --->
    $(document).on("click", ".btn-agregar", function() {
        if($("#validaUsuario").valid()){
            ...
            $.post('admonPri/agregarRol', { nombre: nombre, clave: clave, descripcion: descripcion, prefijo: prefijo, modulo: modulo, vertiente: vertiente}, function(data){      
                if(data > 0){
                    consultarLista();
    ...
    * --->
    window.actionEvents = {        
	    ...
	    $.post('admonPri/consultarUsuario', { 
...
function consultarLista(){
    $.post('admonPri/obtenerLista', function(data){
        $('#lista').html( data );
