...
//	var monitoreoTbl;
	...
	function verAccesoDetalle(obj){
		if(confirm("¿Está seguro de ver el detalle del acceso?")){
			...
			$('#formAcceso').attr('action','/index.cfm/administrador/monitoreo/monitoreo/verSeguimiento');
			$('#formAcceso').submit();
		}
	}
	...
	function getTablaAccesos(){
		$('#cargador-modal').modal('show');
		$.post('/index.cfm/administrador/monitoreo/monitoreo/verTablaRegistros', {numeroPagina:$('#cmbPagina').find("option:selected").val(), ur:$('#cmbUR').find("option:selected").val(), fechaIni:$("#fechaIni").val(), fechaFin:$("#fechaFin").val() }, function(data){
