	...
	<!--
		...
		* --->
		function verLineaError(objeto){
			$.post('/index.cfm/administrador/error/error/verErrorLinea', {nomArchivo: $(objeto).attr('data-file') , numeroLinea: $(objeto).attr("data-line"), fechaError : $(objeto).attr("data-date-error") }, function(data){
		...
		* --->
		function verReferencias(){
			...
			$.post('/index.cfm/administrador/error/error/verReferenciaDB_ORA', function(data){
