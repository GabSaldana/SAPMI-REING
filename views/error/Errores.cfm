...
<!--
	...
	function verErrorDetalle(obj){
		if(confirm("¿Desea ver el detalle del error generado?")){
			...
			$('#formErrores').attr('action','/index.cfm/administrador/error/error/verDetalleError');
			$('#formErrores').submit();
		}
	}
	...
	function eliminarError(obj){
		if(confirm("\u00BFDesea eliminar la informaci\u00F3n?")){
			...
			$.post('/index.cfm/administrador/error/error/eliminarError', {pkError:pkRemove}, function(data){
				if(data > 0){
	...
	function getTablaErrores(){
		...
		$.post('/index.cfm/administrador/error/error/verTablaRegistros', {numeroPagina: $('#cmbPagina').find("option:selected").val(),tipoError: valTipoError, ur:$('#cmbUR').find("option:selected").val(), fechaIni:$("#fechaIni").val(), fechaFin:$("#fechaFin").val() }, function(data){
	...
	function analizarInformacion(){
		if(pkAnalisis.length > 0){
			...
			if(confirm("¿Desea ver el an\u00E1lisis sobre los errores encontrados?")){
				...
				$.post('/index.cfm/administrador/error/error/analisisInformacion', {numeroPagina: $('#cmbPagina').find("option:selected").val(),tipoError: valTipoError, ur:$('#cmbUR').find("option:selected").val(), fechaIni:$("#fechaIni").val(), fechaFin:$("#fechaFin").val(), pkRegistrosAz:pkAnalisis.join(",")}, function(data){
					$('#erroresRegistradosID').html( data );
					$('#cargador-modal').modal('hide');
				});
			}else{
