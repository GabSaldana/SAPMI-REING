...
function mostrarProcedimientos(){
	...
	if(proced != 0){
		$.post('<cfoutput>#event.buildLink("tiempos.tiempos.getFiltros")#</cfoutput>',{
			proced:proced
		},function(data){
...
function mostrarTablaTiempos(){
	...
	if(proced != 0){
		$.post('<cfoutput>#event.buildLink("tiempos.tiempos.getTiempo")#</cfoutput>',{
			proced:proced,
			area:area,
			estado:estado,
			rol:rol,
...
$('body').on('click', '#controlEdos', function(){
	var proced = parseInt($('#proced').val() == undefined || $('#proced').val() == null ||  isNaN($('#proced').val()) || $('#proced').val() == '' ? 0 : $('#proced').val() );
	$.post('<cfoutput>#event.buildLink("historial.historial.getHistorial")#</cfoutput>', {
