...
<script type="text/javascript">
	...
	function obtenDatosAcciones(noSub){
	    $.ajax({
	      ...
	      for(var i=0 ; i < noSub ; i++){
				...
				var frase = 'Proyecto ' + data.DATA.ACCION[i] + '<br/>' + data.DATA.NOMBRE[i];
	...
	function gotoQuiz() {
  	...
  	function creaMenuMobile(i, imagen, frase,numero,total, numProy, nomProy){
		...
		var porcentajeProy = [];
		var color;
		porcentajeProy = <cfoutput>#serializeJSON(session.cbstorage.porcentajeProyectos)#</cfoutput>;
		console.log(porcentajeProy);
		
		if (porcentajeProy[numero-1] == 0)
			color = "red";
		else if (porcentajeProy[numero-1] == 100)	
			color = "green";
		else	
			color = "blue";

		var plantilla = '<div class="row celdaMenu accli" id="'+ numero +'" name="'+ arrF[numero-1] +'" style = "border-bottom: '+grosorBorde+'px solid grey;  ">' +
			...
			'<div class="col-xs-9 col-md-9 col-sm-9">'+
				'<p class="txtCeldaMenu"><strong>'+numProy+'</strong><br/><span>'+ nomProy +'</span><br/><h3><span style="color: '+color+'">Avance: '+porcentajeProy[numero-1]+'%</span></h3> </p>'+
			'</div>'+
		'</div>';
