...
<!--- <cfinclude template="/views/formatosTrimestrales/funcionesVerificarLlenado_js.cfm">
 --->
<!--- Modificiacion para validad los elementos no requeridos ---->
...
<div class="w3-container">
	...
	<form class="form-horizontal" role="form" id="formularioFormatoCap"></form><hr>
...
<form id="downloadComprobanteCap" action="<cfoutput>#event.buildLink('formatosTrimestrales.capturaFT.descargarComprobante')#</cfoutput>" method="post" target="_blank">
    <input type="hidden" id="pkCatFmt"   name="pkCatFmt" value="<cfoutput>#pkCFormato#</cfoutput>">
    <input type="hidden" id="pkColDownCap"  name="pkColDown">
    <input type="hidden" id="pkFilaDownCap" name="pkFilaDown">
</form>
...
<script>
		...
		if (ultimoNivel[i].renderer == "noRender") {
			if (ultimoNivel[i].type == "text") {
				if(ultimoNivel[i].validator == 'cadenaCuatroMilRequerida'){
					...
					$("#formularioFormatoCap").append('<div class="form-group"><label for="'+ultimoNivel[i].valor.replace(/\s+/g,"").replace(/(<([^>]+)>)/ig,"")+'" class="control-label col-sm-2">'+ultimoNivel[i].valor+':</label><div class="col-sm-9"><input type="'+ultimoNivel[i].valor.replace(/\s+/g,"").replace(/(<([^>]+)>)/ig,"")+'" class="required form-control" val-pkcolumna='+ultimoNivel[i].data+'></div></div>');
						divLleno=0;
				}
			} else if (ultimoNivel[i].type == "numeric") {
				if (divLleno==0) {$("#formularioFormatoCap").append('<div class="form-group numeric"></div>');divTotal=0;}
				$("#formularioFormatoCap .numeric:last-child").append('<label for="'+ultimoNivel[i].valor.replace(/\s+/g,"").replace(/(<([^>]+)>)/ig,"")+'" class="control-label col-sm-2">'+ultimoNivel[i].valor+':</label><div class="col-sm-2"><input type="'+ultimoNivel[i].valor.replace(/\s+/g,"").replace(/(<([^>]+)>)/ig,"")+'" class="required form-control" val-pkcolumna='+ultimoNivel[i].data+'></div>');
				divLleno++;
				if (divLleno>1){divLleno=0;}
			} else if (ultimoNivel[i].type == "dropdown"){
				if(ultimoNivel[i].validator == 'seleccionMultiple'){
					$("#formularioFormatoCap").append('<div class="form-group"><label for="'+ultimoNivel[i].valor.replace(/\s+/g,"").replace(/(<([^>]+)>)/ig,"")+'" class="control-label col-sm-2">'+ultimoNivel[i].valor+':</label><div class="col-sm-9"><div class="checkboxes" val-pkcolumna='+ultimoNivel[i].data+'></div></div></div>');
					divLleno=0;
					for (var j = 0; j < ultimoNivel[i].source.length; j++) {
						$("#formularioFormatoCap div[val-pkcolumna="+ultimoNivel[i].data+"]").append('<div class="checkbox checkbox-inline checkbox-primary"><input type="checkbox" id="checkbox-'+ultimoNivel[i].source[j].replace(/\s+/g,"")+'" value='+ultimoNivel[i].source[j].replace(/\s+/g,"_")+'><label for="checkbox-'+ultimoNivel[i].source[j].replace(/\s+/g,"")+'">'+ultimoNivel[i].source[j]+'</label></div>');
					}
				} else if(ultimoNivel[i].validator == 'seleccionUnica'){
					$("#formularioFormatoCap").append('<div class="form-group"><label for="'+ultimoNivel[i].valor.replace(/\s+/g,"").replace(/(<([^>]+)>)/ig,"")+'" class="control-label col-sm-2">'+ultimoNivel[i].valor+':</label><div class="col-sm-9"><div class="radios" val-pkcolumna='+ultimoNivel[i].data+'></div></div></div>');
					divLleno=0;
					for (var j = 0; j < ultimoNivel[i].source.length; j++) {
						$("#formularioFormatoCap div[val-pkcolumna="+ultimoNivel[i].data+"]").append('<div class="radio radio-inline radio-primary"><input type="radio" id="radio-'+ultimoNivel[i].source[j].replace(/\s+/g,"")+'" value='+ultimoNivel[i].source[j].replace(/\s+/g,"_")+' name=radio'+ultimoNivel[i].data+'><label for="radio-'+ultimoNivel[i].source[j].replace(/\s+/g,"")+'">'+ultimoNivel[i].source[j]+'</label></div>');
					} 
				} else if(ultimoNivel[i].validator == 'listaReordenable'){
					$("#formularioFormatoCap").append('<div class="form-group"><label for="'+ultimoNivel[i].valor.replace(/\s+/g,"").replace(/(<([^>]+)>)/ig,"")+'" class="control-label col-sm-2">'+ultimoNivel[i].valor+':</label><div class="col-sm-9"><div class="dd" val-pkcolumna='+ultimoNivel[i].data+'>Ordena las opciones de mayor a menor importancia<ol class="dd-list"></ol></div></div></div>');
					divLleno=0;
					for (var j = 0; j < ultimoNivel[i].source.length; j++) {
						$("#formularioFormatoCap div[val-pkcolumna="+ultimoNivel[i].data+"]").children().append('<li class="dd-item" data-id="'+ultimoNivel[i].source[j].replace(/\s+/g,"_")+'" id="'+ultimoNivel[i].source[j].replace(/\s+/g,"_")+'"><div class="dd-handle"><i class="fa fa-circle text-success"></i> '+ultimoNivel[i].source[j]+'</div></li>');
					}
				} else if(ultimoNivel[i].validator == 'seleccionArchivo'){
					$("#formularioFormatoCap").append('<div id="ftp'+ultimoNivel[i].data+'" class="form-group"><label class="control-label col-sm-2">'+ultimoNivel[i].valor+'</label><div class="col-sm-9"><div id="divInput'+ultimoNivel[i].data+'"><input id="subirArchivo'+ultimoNivel[i].data+'"  pkCol="'+ultimoNivel[i].data+'"  name="upload_files" type="file" multiple class="file-loading subirArchivo"></div> <br> '+getMenuArchivo(ultimoNivel[i].data,ultimoNivel[i].valor)+' <input type="hidden" class="form-control" val-pkcolumna='+ultimoNivel[i].data+'><div style="display:none" class="cancelar'+ultimoNivel[i].data+' text-right"><a onclick="cancelarEdicion('+ultimoNivel[i].data+');" class="btn btn-warning"><i class="fa fa-times"></i> Cancelar</a></div></div>  </div>');
			...
			if (ultimoNivel[i].type == "text") {
				$("#formularioFormatoCap").append('<div class="form-group"><label for="'+ultimoNivel[i].valor.replace(/\s+/g,"").replace(/(<([^>]+)>)/ig,"")+'" class="control-label col-sm-2">'+ultimoNivel[i].valor+':</label><div class="col-sm-9"><input type="'+ultimoNivel[i].valor.replace(/\s+/g,"").replace(/(<([^>]+)>)/ig,"")+'" class="required form-control" val-pkcolumna='+ultimoNivel[i].data+' readonly></div>');
					divLleno=0;
			} else if (ultimoNivel[i].type == "numeric") {
				if (divLleno==0) {$("#formularioFormatoCap").append('<div class="form-group numeric"></div>');divTotal=0;}
				$("#formularioFormatoCap .numeric:last-child").append('<label for="'+ultimoNivel[i].valor.replace(/\s+/g,"").replace(/(<([^>]+)>)/ig,"")+'" class="control-label col-sm-2">'+ultimoNivel[i].valor+':</label><div class="col-sm-2"><input type="'+ultimoNivel[i].valor.replace(/\s+/g,"").replace(/(<([^>]+)>)/ig,"")+'" class="required form-control" val-pkcolumna='+ultimoNivel[i].data+' readonly>');
				divLleno++;
				if (divLleno>1){divLleno=0;}
			} else if (ultimoNivel[i].type == "dropdown") {
				...
				divLleno=0;
				for (var j = 0; j < ultimoNivel[i].source.length; j++) {
					$("#formularioFormatoCap select[val-pkcolumna="+ultimoNivel[i].data+"]").append('<option value='+ultimoNivel[i].source[j].replace(/\s+/g,"")+'>'+ultimoNivel[i].source[j]+'</option>');
				}
			...
			if (divTotal==1) {$("#formularioFormatoCap").append('<div class="form-group numeric"></div>');divTotal=0;}
			$("#formularioFormatoCap .numeric:last-child").append('<label for="'+ultimoNivel[i].valor.replace(/\s+/g,"").replace(/(<([^>]+)>)/ig,"")+'" class="control-label col-sm-1">'+ultimoNivel[i].valor+':</label><div class="col-sm-2"><input type="'+ultimoNivel[i].valor.replace(/\s+/g,"").replace(/(<([^>]+)>)/ig,"")+'" class="required form-control" val-pkcolumna='+ultimoNivel[i].data+' readonly></div>');
			divLleno=0;
			divTotal=1;
		...
		<cfloop array="#encabezado.getColumnas()#" index="columna">
			<cfif columnaUltimoNivel.getPk_Padre() eq columna.getData() and columna.getNom_columna() neq '-'>
				if (!$("#padre-<cfoutput>#columnaUltimoNivel.getPk_Padre()#</cfoutput>").length) {
					$('#formularioFormatoCap [val-pkcolumna="<cfoutput>#columnaUltimoNivel.getData()#</cfoutput>"]').parent().parent().before('<label id="padre-<cfoutput>#columnaUltimoNivel.getPk_Padre()#</cfoutput>" style="margin-left:50px"><cfoutput>#columna.getNom_columna()#</cfoutput></label>');
				}
			</cfif>
		</cfloop>
	</cfloop>
	...
	for (var i = 0; i < sumables.length; i++) {
		for (var j = 0; j < sumables[i].origen.length; j++) {
			$("#formularioFormatoCap input[val-pkcolumna="+sumables[i].origen[j]+"]").attr("sumaDestino",sumables[i].destino);
			$("#formularioFormatoCap input[val-pkcolumna="+sumables[i].origen[j]+"]").attr("origen"+sumables[i].destino,j+1);
			$("#formularioFormatoCap input[val-pkcolumna="+sumables[i].origen[j]+"]").addClass("suma");
		}
		$("#formularioFormatoCap input[val-pkcolumna="+sumables[i].destino+"]").attr("elementosSuma",sumables[i].origen.length);
	}
	...
	$('.suma').change(function() {
		...
		var elementosSuma = $("#formularioFormatoCap input[val-pkcolumna="+destino+"]").attr("elementosSuma");
		var sumatoria = 0;
		for(var i = 1; i <= elementosSuma; i++){
			sumatoria = sumatoria + Number($("#formularioFormatoCap input[origen"+destino+"="+i+"]").val());
		}
		$("#formularioFormatoCap input[val-pkcolumna="+destino+"]").val(sumatoria);
	});
	...
	$("#formularioFormatoCap select[val-pkcolumna="+CatalogoOrigen+"]").change(function() {
		mostrarCatalogo(0);
	});

	function mostrarCatalogo(obtieneDatos){
		$("#formularioFormatoCap select[val-pkcolumna="+CatalogoDestino+"] option").each(function(){
			$(this).show();
		});
		if (obtieneDatos==0) {
			$("#formularioFormatoCap select[val-pkcolumna="+CatalogoDestino+"]").val(0);
		}
		if($("#formularioFormatoCap select[val-pkcolumna="+CatalogoOrigen+"]").val()!=0){
			var valor = $("#formularioFormatoCap select[val-pkcolumna="+CatalogoOrigen+"]").find('option:selected').text();
			$("#formularioFormatoCap select[val-pkcolumna="+CatalogoDestino+"] option").each(function(){
				if ($(this).val()!=0) {
	...
	function saveDatosFormulario(){
		...
		var valor;
		for (var i = 0; i < ultimoNivel.length; i++) {
			if ($("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"]").is("select")) {
				if ($("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"]").val() != 0) {
					valor = $("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"] option:selected").text();
				} else{
					valor = '';
				}
			} else if ($("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"]").is("div")){
				if ($("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"]").hasClass("checkboxes")) {
					var checkboxes = {};
					$("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"] input[type=checkbox]").each(function(){
						checkboxes[$(this).val()] = $(this).is(":checked");
					});
					valor = JSON.stringify(checkboxes);
				} else if ($("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"]").hasClass("radios")) {
					var radios = {};
					$("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"] input[type=radio]").each(function(){
						radios[$(this).val()] = $(this).is(":checked");
					});
					valor = JSON.stringify(radios);
				} else if ($("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"]").hasClass("dd")) {
					valor = JSON.stringify($("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"]").nestable('serialize'));
				}
			} else {
				valor = $("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"]").val();
			}
		...
		return $.ajax({
			...
			success:function(data){
				...
				for (var i = 0; i < data.DATA.COLUMNA.length; i++) {
					$("#formularioFormatoCap input[val-pkcolumna="+data.DATA.COLUMNA[i]+"]").val(data.DATA.VALOR[i]);
					$("#formularioFormatoCap textarea[val-pkcolumna="+data.DATA.COLUMNA[i]+"]").val(data.DATA.VALOR[i]);
					if (data.DATA.VALOR[i] != null){
						if ($("#formularioFormatoCap [val-pkcolumna="+data.DATA.COLUMNA[i]+"]").is("textarea") && $("#formularioFormatoCap [val-pkcolumna="+data.DATA.COLUMNA[i]+"]").hasClass('counterCaracteres1000Max')){
							$("#formularioFormatoCap [val-pkcolumna="+data.DATA.COLUMNA[i]+"]").next().html('Caracteres escritos: '+data.DATA.VALOR[i].length+' de 1000');
						} else if ($("#formularioFormatoCap [val-pkcolumna="+data.DATA.COLUMNA[i]+"]").is("textarea") && $("#formularioFormatoCap [val-pkcolumna="+data.DATA.COLUMNA[i]+"]").hasClass('counterCaracteres4000Max')){
							$("#formularioFormatoCap [val-pkcolumna="+data.DATA.COLUMNA[i]+"]").next().html('Caracteres escritos: '+data.DATA.VALOR[i].length+' de 4000');
						} else if ($("#formularioFormatoCap [val-pkcolumna="+data.DATA.COLUMNA[i]+"]").is("div") && $("#formularioFormatoCap [val-pkcolumna="+data.DATA.COLUMNA[i]+"]").hasClass('checkboxes')){
							checkboxes = JSON.parse(data.DATA.VALOR[i]);
							for(var valores in checkboxes){
								$("#formularioFormatoCap .checkboxes input[value="+valores+"]").attr('checked', checkboxes[valores]);
							}
						} else if ($("#formularioFormatoCap [val-pkcolumna="+data.DATA.COLUMNA[i]+"]").is("div") && $("#formularioFormatoCap [val-pkcolumna="+data.DATA.COLUMNA[i]+"]").hasClass('radios')){
							radios = JSON.parse(data.DATA.VALOR[i]);
							for(var valores in radios){
								$("#formularioFormatoCap .radios input[value="+valores+"]").attr('checked', radios[valores]);
							}
						} else if ($("#formularioFormatoCap [val-pkcolumna="+data.DATA.COLUMNA[i]+"]").is("div") && $("#formularioFormatoCap [val-pkcolumna="+data.DATA.COLUMNA[i]+"]").hasClass('dd')){
							list = JSON.parse(data.DATA.VALOR[i]);
							for(var i = 1; i < list.length; i++){
								$("#"+list[i].id).insertAfter("#"+list[i-1].id);
							}	
						} else if ($("#formularioFormatoCap [val-pkcolumna="+data.DATA.COLUMNA[i]+"]").is("input")){
					...
					if (typeof data.DATA.VALOR[i] === 'string') {
						$("#formularioFormatoCap select[val-pkcolumna="+data.DATA.COLUMNA[i]+"]").val(data.DATA.VALOR[i].replace(/\s+/g,""));
					}
					if(!data.DATA.VALOR[i]) {
						$("#formularioFormatoCap select[val-pkcolumna="+data.DATA.COLUMNA[i]+"]").val(0);
					}
				}
				datosSinModificar = recolectarDatos();
				mostrarCatalogo(1);
	...
	function recolectarDatos(){
		var datos = [];
		for (var i = 0; i < ultimoNivel.length; i++) {
			datos[i] = $("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"]").val();
			if ($("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"]").is("div") && $("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"]").hasClass('checkboxes')){
				var checkboxes = {};
				$("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"] input[type=checkbox]").each(function(){
					checkboxes[$(this).val()] = $(this).is(":checked");
				});
				datos[i] = JSON.stringify(checkboxes);
			} else if ($("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"]").is("div") && $("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"]").hasClass('radios')){
				var radios = {};
				$("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"] input[type=radio]").each(function(){
					radios[$(this).val()] = $(this).is(":checked");
				});
				datos[i] = JSON.stringify(radios);
			} else if ($("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"]").is("div") && $("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"]").hasClass('dd')) {
				datos[i] = JSON.stringify($("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"]").nestable('serialize'));
			}
		}
		return datos;
	}
	...
	$('#formularioFormatoCap').keyup(function(){
		validarFormulario();
	});
				...
				if(typeof ultimoNivel[i].validator !== 'undefined'){
					if(typeof window[ultimoNivel[i].validator] === 'function'){
						...
						if(! window[ultimoNivel[i].validator](valor)){
							$("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"]").addClass('error');
							$("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"]").after('<label id="error-'+ultimoNivel[i].data+'" class="text-danger">Datos invalidos</label>');
							validado = false;
						}
					} else {
						valor = $("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"]").val();
	...
	function limpiarValidaciones(){
		for (var i = 0; i < ultimoNivel.length; i++) {
			$("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"]").removeClass('error');
    ...
    * Comentario: Lanza la modal correspondiente al archivo para cargar archivos.
    ...
    function descargaComprobante(pkcol){
    	$("#pkColDownCap").val(pkcol);
    	$("#pkFilaDownCap").val($("#pkFila").val());
    	$('#downloadComprobanteCap').submit();
    }

    <!---
    * Fecha : Noviembre de 2017
    * Autor : Ana Belem Juárez Méndez
    * Comentario: Guarda nuevo producto.
    --->
    ...
    * Autor : SGS
    * Comentario: Llena los campos precargados.
    --->
    function llenarPrecargados(){
    	for (var i = 0; i < ultimoNivel.length; i++) {
	    	if(catalogoPrecargados.includes(ultimoNivel[i].data)){
	    		celda = ultimoNivel[i].data;
	    		$.ajax({
					type: 'POST',
					url: '<cfoutput>#event.buildLink("formatosTrimestrales.capturaFT.llenarPrecargados")#</cfoutput>',
					data: {
						celda: celda,
						issn: $("#issnPrecargados").val(),
						issnAnio: $("#anio_issnPrecargados").val()
					},
					async: false,
					success: function(data){
						$("#formularioFormatoCap [val-pkcolumna="+ultimoNivel[i].data+"]").val(data).attr('readonly',true);
					}
				});	    		
	    	}
	    }
    ...
    function eliminarComprobante(pkCol){
    	$.post('<cfoutput>#event.buildLink('formatosTrimestrales.capturaFT.eliminarComprobante')#</cfoutput>',{
    		...
    		if(data > 0){
    			...
    			$('#divInput'+pkCol).show();
    			$('#formularioFormatoCap [val-pkcolumna="'+pkCol+'"]').val('');
    		}
    	});
    }
    ...
    * Comentario: Edicion del archivo asociado
    ...
    function getMenuArchivo(pkCol, nombreArchivo){
    	console.log("generacion de menu");
