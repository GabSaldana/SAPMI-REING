	...
	<div class="row">
		<div class="col-md-12">
			<div id="registroForm" class="form-vertical" role="form">
				...
				<br>
			<!---	<div id="secAnalis">									<!-- ABJM  para identificar los datos que no deben ser vistos por un capturista -->
...
/*function trimestresAnteriores(){
	...
	function bloquearparaCaptura(bloqueada){
	  ...
	  if(hot_guardar==1){
	  	if(bloqueada){
	  		document.getElementById("bloqueada").checked = false;
			alert("Antes de bloquear, guarda tus cambios.");
		}else{
			document.getElementById("bloqueada").checked = true;
	  		alert("Antes de desbloquear, guarda tus cambios.");
	  	}
	  }else{
		$.ajax({
			type:"POST",
			...
			success:function(data){
				if(bloqueada && data == <cfoutput>#columna.getpk_columna()#</cfoutput>){
					...
					cargarTabla();													//A.B.J.M. Para que se vean reflejados los cambios de bloqueo de columnas
				}else if(data == <cfoutput>#columna.getpk_columna()#</cfoutput>){
					toastr.success('Desbloqueada','Columna');
					cargarTabla();													//A.B.J.M. Para que se vean reflejados los cambios de desbloqueo de columnas
				}else{
