<cfprocessingdirective pageEncoding="utf-8">
    ...
    <script type="text/javascript">
        ...
        for(var i = 0; i < estados.length; i++){
        	var posx = i * 150;
        	var posy = i%2 * 200;
        	nodes.push({id:estados[i], x:posx, y:posy, label:'Estado '+ estados[i], color: {background:'#fff', border:'#7F2554', highlight:{background:'#68DCFF', border:'#000'}}});
        }

        // create a network
        var container = document.getElementById('mynetwork');

        var edgess = new vis.DataSet(edges);
        var nodess = new vis.DataSet(nodes);

        var data = {nodes: nodess, edges: edgess};
...
<div id="mynetwork" class="col-md-9"></div>
