	 ...
	 $(document).ready(function() {
		 ...
		 $("##contenido").on("click","[data-tab-id] .el-eliminar",
			 function (){
				 ...
				 $.post('#event.buildLink("adminCSII.tablasDinamicas.administradorTablas.eliminarTabla")#',
				     {idTab:id},
				 function(data){
					 if (data > 0){
		 ...
		 $("##contenido").on("click","[data-tab-id] .el-eliminar-compartido",
			 function (){
				 var elemento=$(this).closest("[data-tab-id]");
				 var id=elemento.attr("data-tab-id");
				 $.post('#event.buildLink("adminCSII.tablasDinamicas.administradorTablas.eliminarTablaC")#',
				     {idTab:id},
				 function(data){
					 if (data > 0){
					     var padre=elemento.parent();
		 ...
		 $("##contenido").on("click",".el-copiar-rep",
			 function (){
				 var elemento=$(this).closest("[data-tab-id]");
				 var id=elemento.attr("data-tab-id");
				 $.post('#event.buildLink("adminCSII.tablasDinamicas.administradorTablas.copiarTabla")#',
