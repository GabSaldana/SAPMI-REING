...
function eliminaAccion(){
    ...
    $.post('<cfoutput>#event.buildLink("adminCSII.admonPri.admonPri.eliminaAccion")#</cfoutput>', {
            pkUsu: pk,
        }, function(data){
            if(data > 0){
...
function search(ele){
   ...
   $("#viewIcon").attr("class",clase + icono + " fa-3x" );
}

function actualizarAccion(edo, successMsg, errorMsg) {
    var pk = $('#mdl-estado-accion').find('p').attr('pkAcc');
    $("#mdl-estado-accion").modal('hide');
    $.post('<cfoutput>#event.buildLink("adminCSII.admonPri.admonPri.actualizarAccion")#</cfoutput>', {
            pkUsu: pk, estado: edo
        }, function(data) {
        if(data > 0) {
            consultarAcciones();
...
function consultarAcciones(curso){
    curso = $("#inrol").val();
    $.post('<cfoutput>#event.buildLink("adminCSII.admonPri.admonPri.mostrarAcciones")#</cfoutput>', { 
          curso: curso            
        }, function(data){            
...
$(document).ready(function() {
    ...
    $(document).on("click", ".btn-agregar", function() {
        if($("#validaAccion2").valid()){
            ...
            $.post('<cfoutput>#event.buildLink("adminCSII.admonPri.admonPri.agregarAccion")#</cfoutput>', { modulo: modulo, estado: estado, nombre: nombre,  descripcion: descripcion, orden:orden, clave: clave, icono:icono}, 
            function(data){      
                if(data > 0){
                    consultarAcciones();
    ...
    $(document).on("click", ".btn-editar", function() { 
        if($("#validaAccion2").valid()){
            ...
            var clave       = $("#inClave").val();
            var icono       = $("#inIcon").val();
            $.post('<cfoutput>#event.buildLink("adminCSII.admonPri.admonPri.editarAccion")#</cfoutput>', { 
     ...
     window.actionEvents = {        
        ...
        'click .editar-usuario-validado': function (e, value, row, index) {
            ...
            $.post('<cfoutput>#event.buildLink("adminCSII.admonPri.admonPri.consultarInfoAcciones")#</cfoutput>', { 
