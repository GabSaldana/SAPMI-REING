...
<!------------------------------------------- PUBLIC ------------------------------------------->
	...
	<!---
	* Modificacion : - Se limpia la URL y se identifica el PATH que contiene la URL, posterior se compara el PATH del CGI variables del servidor.
	* Fecha mod : Abril 27, 2015
	* Autor mod : Sergio E. Cuevas Olivares
	--------------------------------------------------
	* Modificacion : Se complementa la comparación de todo el path del componente solicitado y del que se procesa en ese momento
	* Fecha mod : Abril 27, 2015
	* Autor mod : Ricardo Angeles Trujillo
	--------------------------------------------------
	* Fecha : Abril 24, 2015
	* Autor : Sergio E. Cuevas Olivares
	--->
	<cffunction name="getActiveLink" access="public" output="no" returntype="any" hint="Activa la opcion que selecciono el usuario">
		<cfargument name="linkto" 		required="true" 	type="string"  hint="The event or route you want to create the link to">
		<cfscript>
			var className = '';
			var path = '';
			if (cgi.QUERY_STRING != '' && findOneOf(cgi.QUERY_STRING, "0") == 0) //SÓLO PARA PRUEBAS
				path = cgi.PATH_INFO & "?" & cgi.QUERY_STRING;
			else 
				path = cgi.PATH_INFO;
			if(path EQ Mid(buildLink(arguments.linkto),(Len(Trim(ListFirst(buildLink(arguments.linkto),'//'))) + Len('//') + Len(Trim(cgi.HTTP_HOST)) + Len(Trim(cgi.SCRIPT_NAME)))+1, Len(buildLink(arguments.linkto)))){
				className = ' active';
			}
			return className;
		</cfscript>
