...
<cfcomponent>
	...
	<cfproperty name="DAO"			inject="tiempos.DAO_tiempos">
	...
	* Autor:	Roberto Cadena
	...
	<cffunction name="getTiempo" access="remote" hint="Obtiene el tiempo transcurrido de un convenio">
		...
		<cfscript>
			...
			cont = 0;
			if(proced eq 1)
				var convenios = DAO.getAllFormatos(proced);
			else if( proced eq 2)
				var convenios = DAO.getAllTrim(proced);
			else
				var convenios = DAO.getAllDatos(proced);
			for(i = 1; i lte convenios.recordcount; i++){
				var fila = populator.populateFromQuery(wirebox.getInstance("tiempos/B_TIEMPO"),convenios,i);
			...
			if(cont > 1){
				...
				var lastRow = populator.populateFromStruct(wirebox.getInstance("tiempos/B_TIEMPO"),suma,1);
