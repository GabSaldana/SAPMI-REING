...
<cfcomponent accessors="true" singleton>
	<cfproperty name="daoConjuntos" inject="tablasDinamicas/DAO_ConjuntosDatos">
	...
	<cffunction name="obtenerVisualizacionPorId" hint="Obtiene los datos de una visualizacion y contruye su objeto de dominio">
		...
		<cfscript>
			...
			if(!isNull(visualizacionC)){
				...
				var visualizacion=populator.populateFromQuery(wirebox.getInstance("reportesAdhoc/Visualizacion"),consultaVis);
				var tipoVis=populator.populateFromQuery(wirebox.getInstance("reportesAdhoc/TipoVisualizaciones"),consultaTipo);
				...
				for (var x=1; x lte consultaPropiedades.recordcount; x++ ){
					var propiedad=populator.populateFromQuery(wirebox.getInstance("reportesAdhoc/Propiedad"),consultaPropiedades,x);
	...
	<cffunction name="obtenerTiposVis" hint="Obtiene los tipos de visualizacion disponibles">
		<cfscript>
			...
			for(var x=1; x lte consultaTipos.recordcount; x++){
				var tipo=populator.populateFromQuery(wirebox.getInstance("reportesAdhoc/TipoVisualizaciones"),consultaTipos,x);
  	...
  	<cffunction name="agregarComponenteVis" hint="Integra la agregacion  una metrica o dimension a una visualizacion">
		...
		<cfscript>
			...
			if(isNull(columna) ){
				...
				columna= populator.populateFromQuery(wirebox.getInstance("reportesAdhoc/Columna"),consultaCol);
				visualizacion.agregarColumna(columna);
			}
	
					
			var consultaAgr=dao.insertarAgregacionColVis(idVis,idCol,idAgr);
			var agregacion= populator.populateFromQuery(wirebox.getInstance("reportesAdhoc/Agregacion"),consultaAgr);
	...
	<!---
  	...
  	<cffunction name="agregarFiltroVis" hint="Agrega un filtro para una demension a una visualizacion">
		...
		<cfscript>
			...
			var filtro= populator.populateFromQuery(wirebox.getInstance("reportesAdhoc/Filtro"),consultaFlt);
